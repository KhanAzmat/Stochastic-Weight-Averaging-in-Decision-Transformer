...
def experiment(
    ...
    def eval_episodes(target_rew):
        ...
        def fn(model):
            ...
            for _ in range(num_eval_episodes):
                ...
                returns.append(ret)
                #import pudb; pudb.set_trace()
                #returns.append(env.get_normalized_score(ret))
                lengths.append(length)

            writer.add_scalar('target_{target_rew}_return_mean', np.mean(returns))
            writer.add_scalar('target_{target_rew}_return_std', np.std(returns))
            writer.add_scalar('target_{target_rew}_length_mean', np.mean(lengths))
            writer.add_scalar('target_{target_rew}_length_std', np.std(lengths))
            return {
    ...
    swa_start = 500 # 7500
    swa_freq = 250 # 500
    optimizer = torchcontrib.optim.SWA(optimizerAdamW, swa_start=swa_start, swa_freq=swa_freq, swa_lr=variant['learning_rate'])
